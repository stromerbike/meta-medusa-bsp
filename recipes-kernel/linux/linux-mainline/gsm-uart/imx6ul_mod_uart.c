...
static struct uart_driver imx_mod_uart_uart_driver = {
	.owner          = THIS_MODULE,
...
static int imx_mod_uart_probe(struct platform_device *pdev)
{
	...
	printk("Calling mctrl_gpio_init");
	...
	printk("...Calling imx_uart_readl @0x08h", (sport->port.membase + UCR1));
	ucr1 = imx_uart_readl(sport, UCR1);
	ucr1 &= ~(UCR1_ADEN | UCR1_TRDYEN | UCR1_IDEN | UCR1_RRDYEN | UCR1_RTSDEN);
	printk("...Calling imx_uart_writel");
	imx_uart_writel(sport, ucr1, UCR1);
	...
	return uart_add_one_port(&imx_mod_uart_uart_driver, &sport->port);
}

static int imx_mod_uart_remove(struct platform_device *pdev)
{
	...
	return uart_remove_one_port(&imx_mod_uart_uart_driver, &sport->port);
}

static void imx_mod_uart_restore_context(struct imx_port *sport)
{
	unsigned long flags;
...
static void imx_mod_uart_save_context(struct imx_port *sport)
{
	unsigned long flags;
...
static void imx_mod_uart_enable_wakeup(struct imx_port *sport, bool on)
{
	u32 ucr3;
...
static int imx_uart_suspend_noirq(struct device *dev)
{
	...
	imx_mod_uart_save_context(sport);
...
static int imx_uart_resume_noirq(struct device *dev)
{
	...
	imx_mod_uart_restore_context(sport);
...
static int imx_uart_suspend(struct device *dev)
{
	...
	uart_suspend_port(&imx_mod_uart_uart_driver, &sport->port);
	...
	imx_mod_uart_enable_wakeup(sport, true);
...
static int imx_uart_resume(struct device *dev)
{
	...
	imx_mod_uart_enable_wakeup(sport, false);

	uart_resume_port(&imx_mod_uart_uart_driver, &sport->port);
...
static struct platform_driver imx_mod_uart_platform_driver = {
	.probe = imx_mod_uart_probe,
	.remove = imx_mod_uart_remove,
...
static int __init imx_mod_uart_init(void)
{
	int ret;

	printk("Registering new uart-driver");
	printk("Size to be allocated: %d", sizeof(struct uart_state));
	ret = uart_register_driver(&imx_mod_uart_uart_driver);
	...
	ret = platform_driver_register(&imx_mod_uart_platform_driver);
	if (ret != 0) {
		printk("Failed");
		uart_unregister_driver(&imx_mod_uart_uart_driver);
	}

	return ret;
}

static void __exit imx_mod_uart_exit(void)
{
	platform_driver_unregister(&imx_mod_uart_platform_driver);
	uart_unregister_driver(&imx_mod_uart_uart_driver);
}

module_init(imx_mod_uart_init);
module_exit(imx_mod_uart_exit);
